...
from textcounter.counter import TextCounter, CountResult
...
class ReadabilityResult:
    ...
    __slots__ = (
        '_flesch_ease', '_flesch_grade', '_avg_sentence_len',
        '_avg_word_len', '_avg_syllables', '_rating'
    )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self._flesch_ease == other._flesch_ease and
            self._flesch_grade == other._flesch_grade
        )
...
class TextStatistics:
    ...
    __slots__ = (
        '_char_count', '_word_count', '_unique_word_count',
        '_sentence_count', '_paragraph_count', '_avg_word_len',
        '_avg_sentence_len', '_vocab_richness', '_char_freq', '_word_freq'
    )
...
class TextAnalyzer:
    ...
    def word_frequency(
        ...
        if exclude_words:
            exclude_normalized = {
                w.lower() if not case_sensitive else w 
                for w in exclude_words
            }
            frequencies = {
                k: v for k, v in frequencies.items()
                if k not in exclude_normalized
            }
    ...
    def _count_syllables(self, word: str) -> int:
        ...
        if word.endswith('e') and count > 1:
            # Check if it's truly silent (not part of -le)
            if len(word) >= 2 and word[-2] not in 'l':
                count -= 1
        
        # Handle '-le' endings (e.g., "table", "simple")
        if (len(word) >= 3 and 
            word.endswith('le') and 
    ...
    def __str__(self) -> str:
        ...
        return f"TextAnalyzer: {stats.word_count} words, {stats.sentence_count} sentences"
